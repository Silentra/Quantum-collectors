...
- **Rendering ownership** (`profile-ui.js`):
  ...
  - Profile layout (`.profile-layout`): CSS grid with left column `summary → info → cosmetics → appearance` and Achievements spanning the right column beside all four. Cosmetics/Appearance share the left-column width on desktop (not full-bleed under Achievements); stacked single-column order on narrow screens is summary → info → Achievements → cosmetics → appearance.
