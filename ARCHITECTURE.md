...
- **Phase A geometry**: `.card-detail-inner` uses CSS Grid (`12fr / 55fr / 2px / 31fr` rows, art `minmax(32px,…)`). Concept label inside `.card-detail-header`. Art `object-position: center top`. `container-type: size` on shells (for Phase B typography only). Typography still uses legacy rem + `.sci-card` overrides.
