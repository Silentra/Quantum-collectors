...
| **Coverage + pan (valid hybrid)** | **Layered `background-size`, not `max()` on two-value pairs** (invalid). **Chrome:** **`auto 100%`** below **1200px** (height-fill on portrait/narrow); **`145% auto` → `155% auto` → `165% auto`** at **≥1200 / ≥1920 / ≥2560px** (width-tier pan on wide hosts). **Inline + expanded previews:** always **`auto 100%`** (short strip / tall stage). Retired: invalid **`max(125% auto, auto 100%)`**. |
