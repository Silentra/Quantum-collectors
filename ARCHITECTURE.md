...
**Synchrotron (glow):** Detector palette (`data-card-glow="synchrotron"`). Single halo host + one `.synchrotron-ring` child (no span pool): static ambient on halo `::before`; parent ring rotates 10s linear; `::before` = monochromatic packet bodies/tails (tier-gated T1=2, T2=3, T3=4 packets, 10–15% arc each); `::after` = packet heads with `synchrotronHeadPulse` (brightness + scale + blur bloom). Explicit animation longhands only.

| **Shimmer** | `js/card-shimmer.js` | `data-card-shimmer` | `.card-shimmer--face` inside `.card-detail-inner` | Mathematical Aura tier ≥ 1 |

**Layer stack (bottom → top):** `.card-glow--halo` (z0) → `.card-cosmetic-effects` border host (z1) → `.card-glow--molten-embers` hotspots (z1, molten only) → `.card-detail-inner` clipped content (z2) → `.card-glow--molten-drift` / `.card-glow--frost-wisps` (z2, after inner, effect-specific) → shimmer face layer inside inner → badges (z5). Synchrotron mounts `.synchrotron-ring` inside halo at z0.
