...
**Synchrotron (glow):** Detector palette (`data-card-glow="synchrotron"`). Halo host + `.synchrotron-track` with four fixed `.sync-packet` slots (tier-gated T1=2, T2=3, T3=4): static subtle ambient on halo `::before`; each packet animates along a rounded-rect perimeter via CSS Motion Path (`offset-path: inset(… round …)`, `offset-distance` 10s linear, `offset-rotate: 0deg` — horizontal orientation, no tangent rotation); `::before` = tail + stable body; `::after` = leading head with `syncHeadPulse` (brightness + bloom, minimal scale). Even spacing via negative `animation-delay` phase lock. Explicit animation longhands only.
