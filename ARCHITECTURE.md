...
js/
  ...
  card-migration.js  - Batch D3 preview + raw backup; Batch D4 fresh-plan commit helpers (cards/* only)
...
- `/cards/{cardId}` - **Batch D final contract:** `BASE_CARD_DEFINITIONS` (125 in `js/card-data.js`) = canonical bundled base. Firebase `/cards` = **sparse overrides + Firebase-only additions** only. Runtime resolves base ⊕ Firebase layer in `cards.js`. **D4 IMPLEMENTED — AWAITING LIVE VERIFICATION** (bandwidth goal achieved after live empty/sparse post-verify). D3 raw backup + D1 export remain recovery artifacts. Fields on sparse/custom nodes: enabled?, name?, … / full defs for Firebase-only
...
- **Batch D2–D4:** `BASE_CARD_DEFINITIONS` (125) in `js/card-data.js` is the bundled base SoT. `applyCardOverride` merges Firebase `/cards` children (sparse or full) onto base **before** finalize/normalize. `getCard` / `getAllCards` / `getEnabledCards` / `getCardsMap` return the **resolved** catalog. Admin edit of a bundled ID writes a **sparse** Firebase child via `buildCardOverride` (or removes the child when equal to base). Bundled IDs cannot be hard-deleted (disable instead). `seedDefaultCards` is gated when the bundle is present (empty Firebase + bundle = valid catalog). `normalizeConceptTypes` does **not** write Firebase (runtime finalize only). **D4 IMPLEMENTED — AWAITING LIVE VERIFICATION** (Firebase `/cards` → sparse overrides/additions via Admin Convert; fresh re-gather required). Batch D is not complete until live D4 post-verify succeeds.
...
**Glow effect ids (registry):** `void`, `rarity`, `molten`, `winterfrost`, `synchrotron` (`COSMETIC_GLOW_EFFECT_IDS` in `card-glow.js`).
...
**Arctic Mist (glow):** Shimmer-matched cool palette (`data-card-glow="winterfrost"`). Player-facing name **Arctic Mist**; pairs with **Winter Frost** shimmer (`data-card-shimmer="winterfrost"`). Two-layer stack only: (1) `.card-glow--halo.card-glow--winterfrost` — fully static `::before` cool interleaved box-shadow + `::after` ring-masked frost pockets (no breathe/pulse); (2) `.card-glow--frost-wisps` — ten static `.frost-wisp` slow vapor slots (2–4s visible windows, 19–53s irregular cycles, ease-in-out longhands, mounted after artwork at z2; tier-gated T1=4, T2=7, T3=10). Opposite temperament to Molten — quiet cold air, intentional silent gaps.

**Synchrotron (glow):** Detector palette (`data-card-glow="synchrotron"`). Halo host + `.synchrotron-track` with color blob bunches (tier-gated): static subtle ambient on halo `::before`; each bunch member animates along a calibrated rounded-rect perimeter via CSS Motion Path (`offset-path: inset(… round …)`, `offset-distance` 10s linear, `offset-rotate: auto` — tangent-following orientation); beam-pipe insets (`--sync-path-inset-t/r/b/l`, all `-1px`). Soft plasma wake + connective haze; static bright leading head on leaders. Shipping CSS may reference `--vfx-sync-motion-*` / `--vfx-sync-blend-*` with fallbacks when unset. Even spacing via negative `animation-delay` phase lock. Explicit animation longhands only on orbit motion.

| **Shimmer** | `js/card-shimmer.js` | `data-card-shimmer` | `.card-shimmer--face` inside `.card-detail-inner` | Mathematical Aura tier ≥ 1 |

**Layer stack (bottom → top):** `.card-glow--halo` (z0) → `.card-cosmetic-effects` border host (z1) → `.card-glow--molten-embers` hotspots (z1, molten only) → `.card-detail-inner` clipped content (z2) → `.card-glow--molten-drift` / `.card-glow--frost-wisps` (z2, after inner, effect-specific) → shimmer face layer inside inner → badges (z5). Synchrotron mounts `.synchrotron-track` + `.sync-packet` spans inside halo at z0.
...
**Status: COMPLETE + VERIFIED.** Removed obsolete `qc_force_root_loading` emergency root boot. Under locked RTDB rules root `/` reads are denied, so the latch was a false recovery path. Scoped boot + path once/subscribe unchanged. Admin maintenance rebuilds unchanged (later S8d slices).

### Phase — S8d-1 admin canonical-read foundation

**Status: IMPLEMENTED — AWAITING RULE DEPLOYMENT / VERIFICATION.** Pasteable: `qcPersonalAudit.workflowS8d1()`.

- Admin-only parent `.read` on `players`, `trades/direct`, `trades/listings` (`admins/{uid}`). Students still cannot enumerate those parents; known-trade child reads unchanged. No write changes.
- Admin `/players` parent read intentionally returns full player records (including legacy `password` / `activeSession` when present) for trusted teachers.
- Client: [`js/admin-maintenance.js`](js/admin-maintenance.js) `adminLoadCanonical` / `qcAdminMaintenance` — Firebase Auth + registry soft-gate; `complete===true` only for authoritative Firebase reads; empty tree ≠ failure.
- Later rebuilds (S8d-2+) must consume returned `value`/`keys` — **not** `db.getChildren` as canonical truth (cache hydrate is a side effect only).
...
| **Layer** | **`#game-shell-chrome::before` only** (`z-index: -1`, `pointer-events: none`); **do not** use **`container-type: size`** on `#game-shell-chrome` (size containment collapses chrome when height is content-derived) |
| **Coverage + pan (valid hybrid)** | **Layered `background-size`, not `max()` on two-value pairs** (invalid). **Chrome:** **`auto 100%`** below **1200px** (height-fill on portrait/narrow); **`135% auto` → `145% auto` → `155% auto`** at **≥1200 / ≥1920 / ≥2560px** (width-tier pan on wide hosts; ~10% less vertical scale than prior tiers). **Inline + expanded previews:** always **`auto 100%`** (short strip / tall stage). Retired: invalid **`max(125% auto, auto 100%)`**. |
...
- **Rendering ownership** (`profile-ui.js`):
  ...
  - Profile layout (`.profile-layout`): left stack (`.profile-layout-main`: summary → info → cosmetics → appearance) beside Achievements. Show 5/10 are content-sized; Show All (`is-show-all`) caps the right column to the left-stack height with `.ach-profile-list` scrolling internally. Narrow screens use `display: contents` on the left wrapper and order summary → info → Achievements → cosmetics → appearance without the desktop height cap.
